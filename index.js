        ...
        .then(
            function (json) {
                ...
                for (var i = Object.keys(jsondata.title).length; i--; i >= 0) {
                    ...
                    div.innerHTML = (`${compareYear(jsondata["year"][i], year0)}<p class="show-pub ${checkIfEmptyValue("", jsondata["type"][i], "")} ${checkIfEmptyValue("", jsondata["community"][i], "")}">${checkIfEmptyValue("<span class='authors'>", jsondata["co-author(s)"][i], "</span>: ")}<span class="title"><a href="${jsondata["link to publication"][i]}" target="_blank">${jsondata["title"][i]}</a></span>${checkIfEmptyValue(", in <span class='publisher'>", jsondata["publisher (e.g. journal/blog name)"][i], "</span>")}. ${checkIfEmptyValue(`<span class='hashtag hashtag-${jsondata["type"][i]} inactive light-mode'>`, jsondata["type"][i], "</span>")} ${checkIfEmptyValue(`<span class='hashtag hashtag-${jsondata["community"][i]} inactive light-mode'>`, jsondata["community"][i], "</span>")}</p>`);
